...
def filter_view():
    ...
    post_image = filtering(img, np.int(request.form['cutoff_value']), request.form['filter'])
    cv2.imwrite(os.path.join(app.config['UPLOAD_FOLDER'], filename), post_image[0])
